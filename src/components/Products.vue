    ...
    <section id="products">
        <div class="mx-auto max-w-2xl px-4 py-16 sm:px-6 sm:py-24 lg:max-w-7xl lg:px-8">
            <div class="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 xl:gap-x-16">
                <div v-for="(product, index) in productsBase.products" :key="product.id" class="group cursor-pointer">
                        ...
                        <div>
                            ...
                            <div v-else>
                                <div class="m-auto flex border border-2 rounded">
                                    <button @click="decrement(product.id)"><svg class="h-8 w-8" fill="none"
                                    ...
                                    <button @click="increment(product.id)"><svg class="h-8 w-8" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
